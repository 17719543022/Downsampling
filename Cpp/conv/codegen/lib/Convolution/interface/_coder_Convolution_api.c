...
void Convolution_api(const mxArray * const prhs[2], const mxArray *plhs[1])
{
  ...
  emxInit_real_T(&st, &a, 1, true);
  emxInit_real_T(&st, &b, 1, true);
  emxInit_real_T(&st, &c, 1, true);
...
void Convolution_atexit(void)
{
  ...
  Convolution_xil_terminate();
  Convolution_xil_shutdown();
...
/*
 * File trailer for _coder_Convolution_api.c
 *
 * [EOF]
 */
