function [N,D] = RationalApproximation(X,tol)
...
if ~isreal(X)
    if norm(imag(X),1) <= tol*norm(real(X),1)
        X = real(X);
    elseif nargout > 1
        [NR,DR] = RationalApproximation(real(X));
        [NI,DI] = RationalApproximation(imag(X));
        D = lcm(DR,DI);
        N = D./DR.*NR + D./DI.*NI*1i;
        return
    else
        N = char(RationalApproximation(real(X)),' +i* ...',RationalApproximation(imag(X)));
        return
    end
end
if nargout > 1
